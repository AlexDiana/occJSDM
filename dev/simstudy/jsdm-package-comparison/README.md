...
Unlike the original pilot fits, the resolution's outputs are small enough to keep in Git, and the working copy they were produced in was temporary. `stability-resolution-archive/` therefore holds the complete `stability-resolution/` tree (the three penalised deterministic references, gradient-noise checks, curvature outputs including the Hessians, all twelve multistart endpoints with trajectories and process records, the immutable script snapshots and every log), the revised `results/` except the 2 MB CSV duplicate of `predictions.rds`, the revised sjSDM parameter file, and the three original weak-penalty polished endpoints that the checker and attempts script read from `stability/fits`. Together with the training data and truth inside `vignettes/teaching-data/jsdm-comparison.rds`, this is enough to rerun every deterministic step of the resolution without the external archive. Only the native sjSDM fits themselves need the frozen Python environment.
