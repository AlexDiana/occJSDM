...
| sjSDM | R package 1.0.7 from Doug's fork release v0.2.1, commit d2ca508; linear logit, df = 2, PyTorch CPU, float64, no covariance/environment penalty or optimiser weight decay. Mojo was not used or upgraded. |
...
| sjSDM | 7.1 points | 13.6 points | Provisional: some optimisation uncertainty remains |
...
All four mean signed errors are about +1.0 to +1.2 percentage points. That small net error hides individual errors in both directions. The absolute-error column is the better answer to “how far off are the estimated probabilities?”. The direction and size of errors are also saved [by probability band](pilot-results/errors-by-band.csv) and [by species](pilot-results/errors-by-species.csv), with cell counts. Counts are species-by-site comparisons, not independent replicated communities.

![Average absolute error in each probability question](pilot-results/absolute-errors.png)

![Estimated and true probabilities, with identical axes](pilot-results/probability-recovery.png)
...
sjSDM's longer runs have checked training log likelihoods of -483.795, -484.006 and -484.001. Their spread, 0.2115, exceeds the predeclared 0.1 stability check. Their largest difference on the fixed prediction grid is 0.9585 percentage points. The table therefore uses the best training-likelihood fit and labels it provisional. The remaining task is to settle optimisation stability before using the comparison to draw conclusions about differences between packages. We have reached the declared second-attempt budget rather than silently extending it until a preferred answer appears.
...
The [sjSDM follow-up](SJSDM-STABILITY-REPORT.md) did not fully resolve optimisation stability. Doug redirected work to teaching, so further fitting is parked. Lesson N uses the original selected results, explicitly labels sjSDM provisional, and adds readable simulation/fitting/extraction code and probability-scale environmental response curves. Repeated communities, larger sample sizes, a probit-generating scenario, matched prior sensitivity and spatial comparisons remain later work. No package source code was changed by this pilot.
