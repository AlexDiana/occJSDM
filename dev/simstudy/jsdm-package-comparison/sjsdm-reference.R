# Diagnostic deterministic optimisation of exactly the same statistical model.
...
parent <- readRDS(file.path(root, "stability/fits", sprintf("sjSDM-low-rate-start-%d.rds", start)))
label <- sprintf("reference-start-%d", start)
...
fit <- optim(initial, function(t) evaluate(t)$value, function(t) evaluate(t)$gradient,
             method = "BFGS", control = list(maxit = 250, reltol = 1e-10, trace = 1, REPORT = 20))
...
summary <- list(start = start, convergence = fit$convergence, counts = fit$counts,
                ...
                loglik_241 = finer$loglik, max_gradient_161 = max(abs(fine$gradient)),
