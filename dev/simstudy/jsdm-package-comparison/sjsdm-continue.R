...
parent_path <- file.path(root, "fits", sprintf("sjSDM-attempt-2-start-%d.rds", start))
parent <- readRDS(parent_path)
label <- sprintf("sjSDM-low-rate-start-%d", start)
...
settings <- list(parent = basename(parent_path), parent_md5 = unname(tools::md5sum(parent_path)),
  seed = 26092440L + start, epochs = 1500L, block = 300L,
  learning_rate = .0002, sampling = 2000L, optimiser = "RMSprop", weight_decay = 0,
...
result <- tryCatch(withCallingHandlers({
  ...
  fit <- sjSDM(Y = input$y, env = linear(input$x, lambda = 0),
    ...
    control = sjSDMControl(optimizer = RMSprop(weight_decay = 0),
  ...
  reticulate::py_run_string(paste(
    ...
    "m.optimizer = torch.optim.RMSprop(list(m.env.parameters()) + [m.sigma], lr=0.0002, alpha=0.99, eps=1e-8, momentum=0.1, centered=False, weight_decay=0)",
  ...
  for (block in 0:5) {
    if (block > 0) {
      model$fit(as.matrix(cbind(1, input$x)), input$y,
                batch_size = 100L, epochs = 300L, sampling = 2000L,
    ...
    rows[[block+1L]] <- data.frame(start = start, epoch = block*300L,
    ...
    saveRDS(list(parameters = p, history = history, diagnostic = diagnostic,
             ...
             sprintf("%s-epoch-%04d.rds", out, block*300L))
