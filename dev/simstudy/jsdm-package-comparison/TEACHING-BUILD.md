...
- Build occJSDM-lesson-4.Rmd with visible tidyverse code, optional simulation/fitting examples, true-versus-estimated figures, both probability targets, signed/absolute errors, species/band comparisons and diagnostic limitations.
...
The sjSDM stability follow-up was completed: two genuine local maxima were verified by curvature, nine fresh native weak-penalty starts joined the original three, and a selection rule recorded before truth was read chose start 11. The bundle was re-exported from the separately versioned `revised/` results root with the twelve weak-penalty attempts appended to the runtime table, and re-verified with the unchanged verifier. Lesson 4's sjSDM row now uses that fit (new sites 7.1 points, sampled sites 13.3), drops the provisional label, and gains a section teaching the two maxima, including the species 2 and 9 swap and the three-way error comparison. The original pilot tables and figures remain in `pilot-results/`; revised ones are in `stability-resolution-results/`.
