...
6. **Complete the separate spatial and release work.** PR #8 still needs Alex's spatial review and the remaining spatial checks. The optional continuous-noise prior is reviewed separately on branch `codex/continuous-noise-prior`, which follows PR #8; the default decision remains open. Then check the final installed package, refit bundled example results and refresh affected vignette outputs. The non-spatial report does not clear these items.
