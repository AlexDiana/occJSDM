...
**Follow-up completed 19 September:** the [sampling-design comparison](nonspatial-design-recheck.md) tests four field samples, 300 sites, and a control given the true hidden site conditions, keeping six PCR replicates per primer. Sixty additional fits and eight targeted longer checks are complete. More field samples and more sites reduce average absolute occupancy error, but substantial errors remain. The follow-up includes three figures, paired comparisons, sampling costs and the remaining diagnostic and rare-species limits. The original results below are preserved.
