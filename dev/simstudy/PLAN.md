...
## Non-spatial correction and follow-up, 14 September 2026

The [new non-spatial recheck](nonspatial-bias-recheck.md) supersedes the diagnosis in section 21.5. The old q comparison used nominal read-event probabilities instead of the probability of recorded positive reads; the difference explains nearly all the historical high-K mean error. q is sampled from a conjugate Beta conditional, not through a Polya-Gamma update. Historical interval coverage against nominal q truth cannot establish the coverage of the fitted positive-read probability. Collection-coefficient truth also needs conversion to the fitter's standardised scale. The report preserves the rounded historical correction separately from the new fitted results.

The completed study has 230 prior-comparison fits and 20 paired exact-occupancy controls on approved main `80d449d`, with spatial effects absent. It includes three and six PCR replicates per primer as operational comparisons; thirty is diagnostic only. Doug requested errors before release targets, retained the current defaults and stopped two additional longer convergence checks to conserve credits. Those checks, Alex's review, the remaining occupancy and rare-species assessment, the new threshold-greater-than-one defect and final release preparation remain open. See the report's Still needed section for exact next steps.
