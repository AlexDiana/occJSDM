...
# Operational extension requested by Doug: at most six PCR replicates per primer.
# Preserve the existing qtest seeds and all other scenario settings.
for(prefix in c('qnear','qfar')) {
  scenario <- find_scenario(paste0(prefix,'_K3'))
  scenario$K <- 6L; scenario$label <- paste0(prefix,'_K6')
  all_scenarios[[length(all_scenarios)+1L]] <- scenario
}
labels <- c('qnear_K6','qfar_K6')
...
for(label in labels) {
  ...
  for(r in seq_len(reps)) {
    ...
    if(label %in% c('qnear_K6','qfar_K6')) priors$p32 <- list(b_betatheta_slope_var=2,a_p=3,b_p=2,a_q=1,b_q=20)
...
if(workers==1L) {
  ...
  result <- parallel::parLapplyLB(cl,jobs,function(j)tryCatch(run_job(j),error=function(e)list(key=j$key,error=conditionMessage(e))),chunk.size=1L)
