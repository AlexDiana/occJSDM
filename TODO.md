...
**Status, 14 September 2026:** Alex has approved and merged the collection-alignment, RNG and residual-correlation fixes; their records are in *Fixed bugs* 49-51. [PR #8](https://github.com/AlexDiana/occJSDM/pull/8), covering spatial fitting, is the only original code-fix PR still open. The optional continuous-noise prior has been separated from PR #8 into [PR #10](https://github.com/AlexDiana/occJSDM/pull/10). [PR #11](https://github.com/AlexDiana/occJSDM/pull/11), this branch, contains the consolidated TODO and can merge independently. The first two non-spatial investigations are now documented in the [bias recheck](dev/simstudy/nonspatial-bias-recheck.md), including alternative priors. They found incorrect simulation comparisons, remaining occupancy errors and a separate read-threshold defect. They do not clear the release gates. Doug wants the errors reported before choosing non-spatial release targets; defaults are unchanged.

## Required code work (Alex, with Doug validating)

1. **ALEX TO REVIEW: correct spatial fitting and assess its beta evidence.** PR #8, branch `codex/spatial-range-update`, now contains only the spatial correction. The model uses the same complete spatial basis in fitting and prediction, chooses a range using the coefficient model it actually fits, then immediately redraws those coefficients. It restores spatial trait prior means, allows support at every observed location and shares equivalent arithmetic between repeated observations.

    **What to do:** review the [spatial guide and results](https://github.com/AlexDiana/occJSDM/blob/codex/spatial-range-update/dev/simstudy/spatial-beta-recheck.md) and [original derivation and failed examples](https://github.com/AlexDiana/occJSDM/blob/codex/spatial-range-update/dev/simstudy/spatial-range-validation.md). Three informative binary datasets meet Doug's provisional five-point target, with low/middle/high average errors of +2.51/-0.08/-2.38 percentage points. That does not establish accuracy for rare species or every sampling design. Review the support-point advice, retain the weak-information and insufficient-support failures, and complete the separate rare-species assessment. The default priors, support-point count, coordinate scaling and range grid are unchanged. Alex's code/evidence review remains open.

2. **ALEX TO REVIEW: the optional noise prior for continuous responses.** [PR #10](https://github.com/AlexDiana/occJSDM/pull/10), branch `codex/continuous-noise-prior`, is based on PR #8. Merge the spatial correction first, then change PR #10's base to `main` and verify its diff before merging it. It lets users choose a half-Cauchy prior on noise standard deviation, which allows smaller noise values, and configure the existing inverse-gamma prior. The current inverse-gamma default is unchanged. Binary and detection priors are unaffected.

    **What to do:** review the sampler and public settings against the [continuous-noise report](https://github.com/AlexDiana/occJSDM/blob/codex/continuous-noise-prior/dev/simstudy/continuous-noise-prior-validation.md). Three informative continuous datasets using the option recover the true small noise and overall predicted response accurately; weak-information examples still have limitations. Decide whether the option is acceptable and separately whether the default should change. Approval of the option does not itself decide the default or clear the remaining beta checks.

3. **Correct read thresholds greater than one.** The current preprocessing first changes qualifying counts to one, then changes those ones to zero because they are below the requested threshold. As a result, `threshold = 2` or `3` erases all detections. Threshold one, used in the bias recheck, is unaffected.

    **What to do:** create the binary observations directly from the original read counts in one comparison, preserving missing values. Add regression checks for thresholds one, two and three, and verify that a fit using a higher threshold agrees with fitting the correctly pre-thresholded binary data. The [reproducer](dev/simstudy/nonspatial-bias-recheck/q-audit/reproduce_threshold_preprocessing.R) demonstrates the defect in the installed entry point before MCMC. This is an open code fix, separate from prior choices.

## Required bias recheck and release preparation (Doug and Alex)

**JSDM-only sample-size comparison completed 19 September:** the [new report](dev/simstudy/jsdm-sample-size-recheck.md) removes observation error by supplying true presence/absence. At the same original 100 sites, average absolute occupancy-probability error is 11.8 points with 100 fitting sites, 9.7 with 300 and 8.9 with 1,000. Every one of the ten communities improves at each increase; two longer checks change the reported group errors by less than 0.01 point. More sites help, but ten times as many sites reduce average error by about a quarter. This is recovery at fitted sites using the same frozen model version as the earlier checks. It does not test new-site prediction, validate later trait preprocessing, or set a non-spatial release target.

The first two non-spatial investigations have run on approved `main`: 230 fits comparing priors and 20 paired controls given the true occupied/unoccupied states. Each comparison uses ten independent simulated datasets. The two targeted longer convergence checks completed on 19 September for `qfar_K3-q20-08` and `qfar_K3-q9-09`, without warnings. Agreement between chains improved: the worst scored individual Rhat fell from 1.133 to 1.016. Substituting the longer fits changes ten-dataset signed occupancy errors by at most 0.084 percentage points and occupancy-band absolute errors by at most 0.058 points. These checks support the stability of the reported large occupancy errors; they do not resolve those errors, directly recheck K6, or prove perfect convergence of every parameter. Original summaries are preserved, with the comparison reported separately. The report gives the saved commands and all remaining work. Review their results and the remaining decisions below. Spatial checks still need PR #8, with the optional continuous-noise work reviewed afterward. Undercoverage and overcoverage alone can wait until after beta.

**Sampling-design follow-up completed 19 September:** the [new comparison](dev/simstudy/nonspatial-design-recheck.md) keeps six PCR replicates per primer and changes one design feature at a time. Across ten paired communities under each contamination setting, four field samples at 100 sites reduce average absolute occupancy error from 17.9 to 15.3 points under low contamination and from 20.3 to 17.2 under high contamination. Using 300 sites and two samples gives 15.2 and 16.7 points. The main comparison scores the same original 100 sites. All 60 new fits and eight selected longer checks are complete; the report retains two borderline secondary diagnostic flags. The control given the true hidden site conditions also improves errors but does not remove them. No prior or release target changed.

- **Review the non-spatial occupancy and collection results.** With the current collection-slope prior variance of 2, a true 23-point increase in collection success is estimated as about 22 points in the two-stage scenario; variance 0.1 reduces it to about 10 points. The baseline occupancy coefficient has a small average error, but low occupancy probabilities are too high and high probabilities too low. In the two-stage collection scenario those errors average +19.74 and -23.36 percentage points. They partly cancel in the overall average. Supplying the true occupied/unoccupied states roughly halves those errors, showing that imperfect detection makes recovery harder; it does not prove that one prior or code defect explains everything.

    **What to do:** review the [report and paired prior comparisons](dev/simstudy/nonspatial-bias-recheck.md), then decide which probability errors matter scientifically. Keep the current collection prior unless a change improves the relevant quantities together. Review the completed sampling-design comparison before commissioning more simulations. Four field samples achieved most of the overall improvement seen with 300 sites at a smaller increase in field-sample and PCR totals, but low and high occupancy probabilities remain substantially biased. Decide which errors matter scientifically and what further change to investigate if they are unacceptable. The known-site-condition control does not by itself identify a prior or code fix. Respect Doug's operational limit of six PCR replicates per primer; more PCRs alone are not an established remedy. The previous decision in *Fixed bugs* 46 does not close this assessment, and the few rare-species cases need a separate check.

- **Review the corrected false-positive comparison and detection-prior trade-offs.** The old study compared fitted positive-read probabilities with simulated contamination-event probabilities. Some events produce zero reads, creating most of the apparent high-q underestimate. At three PCR replicates per primer, the new high-contamination error is -0.64 percentage points with current priors, rather than the apparent -3.74 points from the old comparison. At the operational maximum of six replicates per primer, current-prior q errors average +0.07 points under low contamination and -0.31 under high contamination, but high-contamination low/high occupancy errors remain +19.41/-24.66 points. The thirty-per-primer runs are diagnostic only, outside the operational PCR limit. Changing q or p priors improves some rates while worsening others and does not solve occupancy errors at the extremes.

    **What to do:** Alex should review the simulation-scoring correction, which now compares p/q with the probability of positive reads and collection coefficients on the fitter's scale. Review q, true-detection p and occupancy together before choosing priors or deciding whether the remaining errors are acceptable. Retain informative assumptions that distinguish true and false detections. The conditional q sampler agrees with an independent Beta calculation; q is not updated through Polya-Gamma sampling. Reassess interval coverage later using the corrected truth, rather than retaining the old coverage claims.
...
- **Test and refresh the package that users will actually receive.** Check the final combined version after the code changes have landed.
    ...
    **Already checked:** on 14 September, before the PR split, the combined PR #8 branch including all three approved fixes and the simulation-scoring correction passed 754 source-test assertions, with no failures, errors or test warnings and one skipped opt-in coverage study. The [non-spatial report](dev/simstudy/nonspatial-bias-recheck.md#reproduction-and-review) records the environment warning and distinguishes this source run from an installed-package check. This does not complete the final release check or refresh the bundled example fit.
...
- **Make the public documentation match the evidence.** Users need to see the remaining limitations in the README, fitting documentation and announcement; this internal TODO is not enough.
    ...
    **Already written on PR #8:** the [fitting vignette](https://github.com/AlexDiana/occJSDM/blob/codex/spatial-range-update/vignettes/occJSDM.Rmd#spatial-effects-inference-and-sampling-design) now explains spatial prediction, limits on ecological inference, sampling grain and extent, replication, and checks on the number of support points. The README links to it. Review this advice alongside the spatial results.
...
49. **Collection covariates aligned with sample identities. APPROVED AND MERGED; recorded 14 September 2026.** Alex approved the change, and [PR #5](https://github.com/AlexDiana/occJSDM/pull/5) is on `main`. One canonical table of typed `(Site, Sample)` pairs supplies the sampler's sample counts and collection-covariate rows in the same order as its observation indices. Identifier covariates and colliding pasted labels no longer disrupt the pairing.

    The 70 alignment assertions cover numeric and character identifiers, shuffled rows, unequal replication, categorical and identifier covariates, and intercept-only models. [Paired recovery results and reproduction instructions](dev/simstudy/collection-alignment-validation.md) retain the positive, zero and negative slope comparisons with unchanged priors. The separate occupancy-baseline, collection-effect and high-false-positive-rate bias rechecks remain open.

50. **Sampler random draws kept on the main thread. APPROVED AND MERGED; recorded 14 September 2026.** Alex approved [PR #6](https://github.com/AlexDiana/occJSDM/pull/6); [PR #9](https://github.com/AlexDiana/occJSDM/pull/9) brought those reviewed changes from the collection branch into `main`. The live Polya-Gamma and coefficient sampling paths run serially, and the C++ RNG uses one advancing R-seeded stream. All modelling features remain available; deterministic calculations can still run in parallel.

    The 29 regression assertions check reproduction across requested thread settings and continued consumption of new draws. [The RNG validation report](dev/simstudy/rng-safety-validation.md) records the call-path audit, instrumented checks and independent distribution checks. Serial sampling is the beta contract; parallel RNG design remains future work. This closes the RNG defect without establishing nominal interval coverage or closing the separate point-bias checks.

51. **Residual species covariance preserved during factor reparameterisation. APPROVED AND MERGED 14 September 2026.** Alex approved [PR #7](https://github.com/AlexDiana/occJSDM/pull/7), merged into `main` as `7856029`. The transform now uses an orthogonal rotation and sign changes, preserving both the fitted factor contribution and the loading covariance. The factor and trait call sites use the correction, including one-factor, rectangular and rank-deficient cases. Biplot documentation no longer claims that anchor loadings are fixed to one.

    The 235 focused expectations and [paired recovery comparisons](dev/simstudy/factor-correlation-validation.md) check covariance, linear predictors, output functions and prediction scale. Mean absolute correlation error in the recorded comparisons fell from 0.247 to 0.021 for continuous data and from 0.179 to 0.054 for binary data. This fixes an output distortion without changing the sampler. Previously normalised fits require refitting or retained raw draws to recover the discarded scales. Other bias and release checks remain open.
