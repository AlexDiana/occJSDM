...
4. **ALEX TO REVIEW: make spatial fitting internally consistent; retain the full recovery gate.** Implemented on branch `codex/spatial-range-update`, based on the RNG fix. The range update integrates the Gaussian coefficient block under each candidate basis, selects the range and immediately redraws those coefficients. Fitting and prediction share the full whitened support-point basis and numerical jitter, and all coefficient samplers use the spatial trait prior means they calculate. Routine fitting no longer computes unused full-GP inverses. One-knot fits and spatial prediction with zero residual factors retain their matrix dimensions.

    **Review:** inspect the conditional-likelihood derivation, covariance invariants and paired full-fit evidence in [the spatial validation report](dev/simstudy/spatial-range-validation.md). The current simulator reproduces the original upper-boundary failure. The demonstrated mismatches concern the full fitter's coefficient/basis representation; the fixed-field result in *Fixed bugs* 48 remains a separate diagnostic. Priors, coordinate scaling and the range grid are unchanged.

    **Before closing:** Alex must review the implementation and evidence, and several sufficiently informative datasets with distinct interior ranges must recover spatial-field and occupancy pattern and level. Use the actual fitted coordinate scale and a fixed knot count. A moving chain alone is insufficient. The original 25-knot example still favors the upper boundary. In 100-site low-noise checks, the existing inverse-Gamma(5,5) noise-variance prior accompanies field slopes of 0.415 to 0.725 against truth. Diagnose knot adequacy and this prior sensitivity, then validate a remedy on spatial magnitudes, range and probability levels without transferring bias between noise and field. Exact range recovery in every replicate and nominal interval coverage are not required. Keep this item open until that gate clears.
