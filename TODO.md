...
**Status, 14 September 2026:** Alex has approved and merged the collection-alignment, RNG and residual-correlation fixes; their records are in *Fixed bugs* 49-51. [PR #8](https://github.com/AlexDiana/occJSDM/pull/8), covering spatial fitting, is the only original code-fix PR still open. The first two non-spatial investigations are now documented in the [bias recheck](dev/simstudy/nonspatial-bias-recheck.md), including alternative priors. They found incorrect simulation comparisons, remaining occupancy errors and a separate read-threshold defect. They do not clear the release gates. Doug wants the errors reported before choosing non-spatial release targets; defaults are unchanged.

## Required code work (Alex, with Doug validating)

1. **ALEX TO REVIEW: correct spatial fitting and check that its estimates are accurate enough for beta.** Implemented in PR #8, on branch `codex/spatial-range-update`, building on the RNG fix now on `main`. The model now evaluates each spatial range with the same coefficient model it actually fits, then redraws the coefficients at the selected range. Fitting and prediction use the same complete spatial basis, including the intended trait effects. The fitter can now use a support point at every observed location; it previously silently omitted one. Repeated observations share an equivalent, faster spatial calculation. Continuous-data users can choose a noise prior that allows small noise values, while retaining the existing inverse-gamma option.

    **Review:** see the [plain-language follow-up and new evidence](dev/simstudy/spatial-beta-recheck.md), plus the derivation and earlier failed examples in [the original spatial validation](dev/simstudy/spatial-range-validation.md). The new noise-prior option applies only to continuous responses. The noise-prior default is still awaiting a decision; the default support-point count, other priors, coordinate scaling and range grid are unchanged. The fixed-field result in *Fixed bugs* 48 remains a separate diagnostic.

    **Checks completed:** three independently simulated, informative binary datasets meet Doug's provisional five-point target: average errors are +2.51, -0.08 and -2.38 percentage points at low, medium and high true probabilities. The report gives uncertainty in those averages and shows the remaining weakening of estimated spatial effects. Three informative continuous datasets using the alternative noise prior recover the true noise level and the overall predicted response accurately. These checks use a support point at every observed location and do not include rare species.

    **Before closing:** Alex should review the code and results and choose the continuous-noise default. Complete the separate rare-species assessment and review the [new sampling-design advice](vignettes/occJSDM.Rmd#spatial-effects-inference-and-sampling-design), including checking that increasing the number of support points does not materially change the answer. Keep the earlier weak-information and insufficient-support examples visible: moving chains or accurate site rankings alone do not resolve their errors. Exact range recovery in every dataset and nominal interval coverage are not required. Keep this item open until the evidence and review support closing it.

2. **Correct read thresholds greater than one.** The current preprocessing first changes qualifying counts to one, then changes those ones to zero because they are below the requested threshold. As a result, `threshold = 2` or `3` erases all detections. Threshold one, used in the bias recheck, is unaffected.
