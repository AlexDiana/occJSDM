...
2. **ALEX TO REVIEW: ensure random draws on the public fitting path are safe and independent.** Implemented on branch `codex/serial-sampling-rng`. The collection and JSDM Polya-Gamma sampling bodies now execute directly on the main thread, including when several TBB threads are requested. The two dormant RNG-bearing worker helpers also run serially. `src/rng.h` uses one advancing R-seeded C++ stream; it no longer derives supposedly independent TBB streams from OpenMP thread IDs. All modelling features remain available, and deterministic probability/sufficient-statistic workers remain parallel.
    ...
    **Validation:** `dev/simstudy/rng-safety-validation.md` records the live call-path audit, analytic distribution checks and a separate instrumented package run. The latter observes every C++ RNG leaf on the main thread across six fitting configurations, including splines and intercept-only fits. Independent PG and normal checks each use 32,768 draws and find no repeated draws or material moment/correlation discrepancy. The full source suite and installed-package tests pass; the obsolete known-RNG-gap skip has been removed.
...
4. **ALEX TO REVIEW: correct spatial fitting and check that its estimates are accurate enough for beta.** Implemented in PR #8, on branch `codex/spatial-range-update`, building on the RNG fix now on `main`. The model now evaluates each spatial range with the same coefficient model it actually fits, then redraws the coefficients at the selected range. Fitting and prediction use the same complete spatial basis, including the intended trait effects. The fitter can now use a support point at every observed location; it previously silently omitted one. Repeated observations share an equivalent, faster spatial calculation. Continuous-data users can choose a noise prior that allows small noise values, while retaining the existing inverse-gamma option.
    ...
    **Before closing:** Alex should review the code and results and choose the continuous-noise default. Complete the separate rare-species assessment and explain how users should check that they have enough support points; increasing the number should not materially change the answer. Keep the earlier weak-information and insufficient-support examples visible: moving chains or accurate site rankings alone do not resolve their errors. Exact range recovery in every dataset and nominal interval coverage are not required. Keep this item open until the evidence and review support closing it.
