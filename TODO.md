...
- **Check whether occupancy and collection effects are now estimated correctly.** `B0` controls a species' baseline tendency to occupy a site. Earlier simulations estimated it too low. Tightening the collection-effect prior helped `B0`, but also pulled genuine collection effects too strongly towards zero.
    ...
    **What to do:** rerun the comparisons with the corrected sample alignment, using the same simulated datasets for each prior setting. Check `B0`, positive and negative collection effects separately, and predicted occupancy probabilities together. Start with Alex's current collection-slope prior variance of 2. Do not switch to 0.1 simply because it improves `B0`. If important bias remains, identify its cause and check that any remedy improves estimates without making another part of the model worse. The earlier prior decision in *Fixed bugs* 46 does not replace this check.

- **Check whether the model still underestimates higher false-positive rates.** `q` is the probability of a positive PCR result when the species' DNA is absent from the sample. Earlier simulations with true rates of 15-30% underestimated them by about 3-4 percentage points, even with more PCR replicates.

    **What to do:** after the alignment and random-number fixes, repeat the existing `qnear_K3`, `qnear_K30`, `qfar_K3` and `qfar_K30` simulations. These compare low and high false-positive rates with 3 or 30 PCR replicates. If the underestimate persists, identify whether the prior or another model problem causes it. Check that any remedy also preserves sensible true-detection rates (`p`) and occupancy estimates. Keep the informative assumptions needed to distinguish true from false detections; simply making every prior less informative is not a safe remedy. Investigate `q` separately: its update is not a Polya-Gamma draw, so similar interval problems do not establish a shared cause.
...
- **Collection-slope and near-prior `q` undercoverage:** defer the remaining interval-width investigation once the point-estimate checks above clear. Keep the two mechanisms separate until evidence connects them. The collection row-alignment defect is fixed (see *Fixed bugs* 49); its separate point-estimate recheck remains required before beta.
