...
2. **ALEX TO REVIEW: the optional noise prior for continuous responses.** Branch `codex/continuous-noise-prior` is a separate PR based on PR #8, so merge the spatial correction first. It lets users choose a half-Cauchy prior on noise standard deviation, which allows smaller noise values, and configure the existing inverse-gamma prior. The current inverse-gamma default is unchanged. Binary and detection priors are unaffected.
