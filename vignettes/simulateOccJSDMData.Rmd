...
- `list_jsdmParams`: JSDM structural parameters -- `gt` (latent trait dimensions), `d` (latent factor dimensions capturing residual species co-occurrence), `ds` (spatial latent factor dimensions), `sigma_b`/`sigma_bs`/`sigma_ts`/`sigma_h` (variance components), and `l_s` (spatial length scale).
...
We rename `sim` to `sim_spat`, to remind ourselves that this dataset has spatial factors. The spatial component uses `ds` latent spatial factors and length scale `l_s` to build a Gaussian-process-correlated random field over `Xs`, which is added to each species' linear predictor for occupancy.
...
Each simulation's `true_params$jsdmParams_true$varPart` decomposes the linear predictor's variance per species into Environmental, Spatial, and Biotic components (plus a Total row of residual/intercept variance). This is a quick way to check that turning on `useSpatField` actually contributes spatial variance.
...
As expected, the "Spatial" column is (numerically) negligible when `useSpatField = FALSE`, and becomes a non-trivial share of variance -- though it varies considerably across species -- when `useSpatField = TRUE`.
...
To make the spatial signal visually dominant, increase `l_s` (broader, smoother spatial patches) and/or reduce `sigma_h`/`sigma_b` (less non-spatial noise from latent factors and environmental covariates).
...
where `A` and `C` represent an *unobserved* trait dimension and its response, and `Bt` is residual, trait-independent variation. `B` (`ncov_psi` x `S`) is the matrix of true environmental-covariate coefficients actually used to simulate occupancy.
...
The sign and magnitude of the slope is governed by `G_true[2, 1]` (trait 2's effect on covariate 1) -- shown above to be nonzero, so a relationship is expected here, with its sign matching `G_true[2, 1]`, attenuated by the unobserved-trait pathway (`A %*% C`) and residual noise (`Bt`). Had `G_true[2, 1]` been `0` instead, species' coefficients would vary with trait 2 only through those other pathways, and no systematic relationship would be expected.
