...
Doug approved a saved-results calibration extension on 25 September 2026, including occJSDM, gllvm, Hmsc and sjSDM. Lesson 4 sections 14-15 add community-level probability bias and RMSE, Bayesian marginal-probability interval coverage at five shared environmental settings, compatible environmental-coefficient recovery, and existing trait intervals. The analysis retains flagged fits, reports passed-only sensitivity summaries, and leaves unavailable uncertainty as missing. It uses the September fits without refitting or pooling the historical July/August study. The compact calibration bundle renders without the fitting archive; its reproducible exporter and verification record are in `dev/simstudy/jsdm-package-comparison/extension/CALIBRATION.md`. More independent communities, gllvm/sjSDM uncertainty methods and probit-generated data remain future work.
