...
```{r}
data(sampleresults) # nburn=5000 and niter=5000
...
### Baseline Occupancy, Collection, Detection, and False-Positive rates

We can plot baseline occupancy, collection, detection, and
false-positive rates for Stage 1 (eDNA collection in the field) and
Stage 2 (eDNA detection in the lab).

We note that baseline occupancy and collection rates are the occupancy
and collection probabilities when fixing all covariates at their mean
levels.
...
Check the help of this function for info

```{r}
returnOrdinationScores(fitmodel)
plotOrdinationScores(fitmodel)

returnFactorLoadings(fitmodel)
