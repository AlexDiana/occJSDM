...
run, we have set `nburn` and `niter` to very low values, which accounts
for the low minimum ESS \~ 40. COMMENT(Save the fitmodel object and do not run this)
...
We note that if a categorical covariate is used, the design matrix will contain
the new levels for that categorical covariates and therefore the names will changes. 
To check the new covariates names, type 
```{r}
...
We can plot occupancy, collection, detection, and false-positive rates for Stage 1
(eDNA collection in the field) and Stage 2 (eDNA detection in the lab). 

We note that occupancy and collection rates are the occupancy and collection
probability when fixing each covariate at its mean level.
...
occJSDM, the conditional occupancy probability is the occupancy probability
accounting for both the covariates *and* the observed detection data
at each site. 
```{r}
...
sites, and $S$ is the number of species, and they contain the posterior means of 
the conditional/predictive occupancy probability.
...
COMMENT(HAve this as an additional output function)
...
tbl <- latentPresences_banded |>
  dplyr::select(Site, Sample, Primer, OTU, CondOccProb,
                ...
                X_psi.1, X_psi.2, X_theta, Xs.1, Xs.2, row_color) |>
  gt() |>
  fmt_number(columns = c(CondOccProb, CondSampleProb,
                         PredOccProb, CollectionProb, DetectionProb,
                         X_psi.1, X_psi.2, X_theta, Xs.1, Xs.2), decimals = 2) |>
  cols_hide(row_color) |>
  tab_header(
