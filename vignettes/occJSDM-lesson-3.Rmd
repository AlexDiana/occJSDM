...
The old walkthrough extracted WAIC to compare model specifications. The following table preserves the **legacy** scores saved with these fits. Request that old score explicitly:
...
**Do not use this table to choose the better model for unsurveyed sites.** The legacy calculation combines likelihood terms for the sampled, unobserved site and collection states with terms for the PCR observations. Those hidden states are learned using the training observations. It does not average them out to evaluate the probability of new observations at a new site. Matching the training dataset is necessary for comparison, but does not by itself fix this difference in target. The independent-site scores above provide the worked predictive comparison. The corrected `computeSiteWAIC()` function instead scores the observed survey at each site, averaging over unobserved occupancy, collection and shared site factors. The default `extractWAIC()` now returns this corrected score; it no longer returns the legacy values in the table. These old fits require `threshold = 1` because they did not save their fitting threshold.

```{r corrected-site-waic, eval=FALSE}
...
The comparison reports the first score minus the second: a positive difference favours the second model. Its standard error describes uncertainty across sites. It does not measure uncertainty from finite MCMC runs or numerical integration. Inspect the warnings and the per-site `p_waic` column before interpreting a ranking. Checks of these saved teaching fits trigger reliability warnings at almost every site, even after integrating the hidden states correctly. **We therefore do not select a factor count here.** An actual held-out-site comparison of the observed surveys remains necessary before teaching a confident choice. The existing independent-site exercise evaluates occupancy probabilities against simulated truth and answers an additional question.
...
WAIC is another calculated quantity with no generating parameter to overlay. The perfect-observation and PCR fits here have different response data, so their WAIC values must not be compared as if they were competing models of one dataset. The legacy stored score includes likelihood terms for sampled, unobserved occupancy and collection states. The corrected site-WAIC function averages over those states and the shared site factors, but still needs reliability checks. The new-site section above shows both interfaces and explains why these teaching fits do not yet justify selecting a factor count.
