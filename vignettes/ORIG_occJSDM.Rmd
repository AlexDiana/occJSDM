...
**New to occJSDM? Start with [Lesson 1: Fit the model and compare its answers with truth](occJSDM-first-lesson.html), or first read [Lesson 0: Create and explore a simulated survey](occJSDM-lesson-0.html).** Both show the R code. Lesson 1 compares estimates with known truth and works through weak true detections, laboratory false positives, strong true detections and field-stage contamination. It also explains how priors express expectations about good field and laboratory practice. [Lesson 2: Spatial landscapes and dispersal](occJSDM-lesson-2.html) is a planned extension pending review of PR #8, with smooth environmental gradients and a separate experiment for species with different dispersal abilities.

The walkthrough below remains a reference tour of the output functions. Its older saved examples do not contain their generating truth and are separate from the new lesson's verified simulation and fits.
...
values) (see Supplementary Information 12 in Ji et al. 2025).

Good field and laboratory practice motivates priors that favour uncommon false positives, but these are starting expectations, not guarantees. The model does not require all false positives to be weak or prohibit an absence inference after repeated positives. In the current thresholded model, evidence comes from positive/negative patterns across PCRs and field samples, rather than the size of a read count above the threshold. A contaminated field sample can produce many positive PCRs even when the species is absent from the site. The [worked truth-based lesson](occJSDM-first-lesson.html#reveal-the-truth-and-compare-it-with-the-fit) shows an actual simulated example, including a mistaken site-presence inference.
