...
The complete experiment has **640 package/model combinations**, before
optimisation restarts: 400 ecological comparisons and 240 trait
comparisons. The following status is part of the saved results. It
prevents an unfinished run from looking like a completed benchmark.
...
extension_status <- as_tibble(extension$manifest) |>
  ...
  summarise(
    ...
    passed_diagnostics = sum(diagnostic_pass),
    scored = sum(scored),
    .groups = "drop"
  )

knitr::kable(extension_status)
```

| package | planned | completed | passed_diagnostics | scored |
|:--------|--------:|----------:|-------------------:|-------:|
| Hmsc    |     180 |       180 |                 94 |    180 |
| gllvm   |     180 |       180 |                133 |    166 |
| occJSDM |     180 |       180 |                180 |    180 |
| sjSDM   |     100 |       100 |                 98 |    100 |
