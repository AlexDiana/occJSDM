...
7. Distinguish bias from error size, and assess interval coverage alongside interval width.

The code is visible in the worked lesson; sections 14-15 present the saved results as a report, with the rendering code in this R Markdown source. Knit this file, or run its chunks with `vignettes` as the working directory. Rendering reads a compact results bundle and does **not** fit any models. The optional simulation and fitting chunks are shown with `eval=FALSE`; run them deliberately if you want to repeat the experiment.
...
This does not establish a generally superior package, prove that any fitted optimum is global, or demonstrate systematic bias across repeated communities. Sections 12-15 add independent simulated communities and sample-size comparisons. A broader study still needs both logit- and probit-generating scenarios and sensitivity to priors. Section 13 adds a matched trait example. Spatial effects, residual-correlation comparisons and variation partitioning require their own matched experiments; they are not included in this pilot.
